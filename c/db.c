...
// 常にルートノードを見るようになっている
// ルートノードが中間ノードだと行のデータを持っていないので，おかしくなる
...
/*
 * Return the position(ノードへのカーソル) of the given key.
